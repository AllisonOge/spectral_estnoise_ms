...
def estnoisem(pSpectrum, hop_length):
    ...
    (nFrames, nFFT2) = np.shape(pSpectrum)
    # initialize output arrays
    x = np.array(np.zeros((nFrames, nFFT2)))
    # will hold std error in the future
    xs = np.array(np.zeros((nFrames, nFFT2)))

    # default algorithm constants
    # smoothing time constant for alpha_c = -hop_length/log(0.7) in equ (11)
    taca = 0.0449
    # max smoothing time constant in (3) = -hop_length/log(0.96)
    tamax = 0.392
    # min smoothing time constant (upper limit) in (3) = -hop_length/log(0.3)
    taminh = 0.0133
    tpfall = 0.064   # time constant for P to fall (12)
    # max smoothing time constant in (20) = -hop_length/log(0.8)
    tbmax = 0.0717
    ...
    qith = np.array([0.03, 0.05, 0.06, np.Inf], dtype=float)
    # maximum permitted +ve noise slope in dB/s
    nsmdb = np.array([47, 31.4, 15.7, 4.1], dtype=float)

    # derived algorithm constants
    # smoothing constant for alpha_c in equ (11) = 0.7
    aca = np.exp(-hop_length/taca)
    ...
    aminh = np.exp(-hop_length/taminh)
    ...
    actminsub = np.array(np.ones(nFFT2) * np.Inf)
    ...
    for t in range(0, nFrames):                       # we use t instead of lambda in the paper
        # noise speech power spectrum
        pSpectrum_t = pSpectrum[t, :]
        acb = (1+(sum(p) / sum(pSpectrum_t)-1) **
               2)**(-1)    # alpha_c-bar(t)  (9)
        ...
        ah = amax*ac*(1+(p/sn2-1)**2)**(-1)
        SNR = sum(p)/sum(sn2)

        # lower limit for alpha_hat (12)
        ah = max_complex(ah, min_complex(
            np.array([aminh]), np.array([SNR**SNRexp])))

        # smoothed noisy speech power (3)
        p = ah*p+(1-ah)*pSpectrum_t

        # smoothing constant for estimating periodogram variance (22 + 2 lines)
        b = min_complex(ah**2, np.array([bmax]))
        pb = b*pb + (1-b)*p            # smoothed periodogram (20)
        pb2 = b*pb2 + (1-b)*p**2     	 # smoothed periodogram squared (21)

        qeqi = max_complex(min_complex((pb2-pb**2)/(2*sn2**2), np.array(
            [qeqimax])), np.array([qeqimin/(t+1)]))   # Qeq inverse (23)
        ...
        if subwc > 1 and subwc < nv:              # middle of buffer - allow a local minimum
            ...
            lminflag = np.logical_or(lminflag, kmod)
            ...
            if subwc >= nv:                   # end of buffer - do a buffer switch
                ...
                lmin = np.logical_and(np.logical_and(np.logical_and(
                    lminflag, np.logical_not(kmod)), actminsub < (nsm*pminu)), actminsub > pminu)
                if any(lmin):
        ...
        xs[t, :] = sn2*np.sqrt(0.266*(nd+100*qisq)*qisq /
                               (1+0.005*nd+6/nd)/(0.5*qeqi**(-1)+nd-1))
